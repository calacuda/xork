...
use bevy::{color::palettes::tailwind::AMBER_500, prelude::*, text::TextPipeline};
...
pub fn setup_all_items_inventory_menu(
    ...
    // let box_size = Vec2::new((10. / 16.) * 1000.0, (8. / 9.) * 1000.0);
...
pub fn display_items(
    ...
    // for (entity, text_layout_info) in &text_q {
    //     // info!("entity +> {}", cmds.entity(entity).id());
    //     let size = text_layout_info.size;
    //     // info!("size => {size}");
    // }
    //
    // for (entity, text_layout_info) in &node_q {
    //     // info!("entity +> {}", cmds.entity(entity).id());
    //     let size = text_layout_info.height;
    //     info!("size => {size:?}");
    // }
    ...
    // let node_h = node_q.get_single().unwrap().height;
